class SideView extends JSView
    ...
    viewDidAppear:->
        ...
        @sourceview = new JSTableView(JSRectMake(0, @tabview._frame.size.height, @_frame.size.width, @_frame.size.height - @tabview._frame.size.height - 24))
        @sourceview.delegate = @sourceview.dataSource = @_self
    ...
    didSelectRowAtIndexPath:(num, e)->
        ...
        if (@lastedittab?)
            @sourceview.cellForRowAtIndexPath(@lastedittab).setBackgroundColor(JSColor("clearColor"))
        cell = @sourceview.cellForRowAtIndexPath(num)
